...
class InspectorPanel extends LegacyElementMixin(PolymerElement) {
  ...
  private generatedImage: HTMLDivElement;
  ...
  ready() {
    ...
    this.generatedImage = this.$$('div.generated-image') as HTMLDivElement;
  ...
  private averageEmbedding() {
    const embeddingSize = this.projector.dataSet.dim[1];
    const embeddingCount = this.selectedPointIndices.length;
    const k = 1.0 / embeddingCount;
    const result = new Float32Array(embeddingSize);

    for (let j = 0; j < embeddingCount; j++) {
      const index = this.selectedPointIndices[j];
      const embedding = this.projector.dataSet.points[index].vector;
      for (let i = 0; i < embeddingSize; i++) {
        result[i] += embedding[i] * k;
      }
    }

    return result;
  }
  private float32ArrayToString(embedding: Float32Array) {
    let str = '[';
    for (let i = 0; i < embedding.length; i++) {
      str += embedding[i].toFixed(2); // round to 2 decimal places
  ...
  private onGenerate() {
    let spinner = this.$$('.generation-sandbox paper-spinner-lite') as any;
    let button = this.$$('.generation-sandbox paper-icon-button') as any;
    const self = this;
    const embedding = this.averageEmbedding();
    const xhr = new XMLHttpRequest();
    const generationUrl =
      this.projector.getRootNode().host.generatorUrl ||
      'https://httpbin.org/post';
    console.log(
      'Posting url=%s embedding=%s',
      generationUrl,
      this.float32ArrayToString(embedding)
    );
    xhr.open('POST', generationUrl);
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.onload = function () {
      if (xhr.status === 200) {
        const generation = JSON.parse(xhr.responseText) as GenerationMetadata;
        const generatedImageUrl = generation.imageUrl || generation.json.url;
        console.log('Got: %o', generation);
        self.generatedImage.style.backgroundImage = `url(${CSS.escape(
          generatedImageUrl!
        )})`;
      } else {
    ...
    xhr.send(
      JSON.stringify({
        embedding: embedding,
        url: 'https://icons.iconarchive.com/icons/designbolts/rio-2-movie/512/Rio2-Carla-icon.png',
      })
    );
